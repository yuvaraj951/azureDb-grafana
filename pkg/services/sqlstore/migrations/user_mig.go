// for all database use instead of DB_Int You have to DB_BigInt expect mssql
...
func addUserMigrations(mg *Migrator) {
	userV1 := Table{
		...
		Columns: []*Column{
			...
			{Name: "account_id", Type: DB_Int, Nullable: false},
	...
	addDropAllIndicesMigrations(mg, "v2", userV2)

	addTableRenameMigration(mg, "user", "user_v2", "v2")
	...
	userV3 := Table{
		...
		Columns: []*Column{
			...
			{Name: "email", Type: DB_NVarchar, Length: 255, Nullable: false},
			{Name: "name", Type: DB_NVarchar, Length: 255, Nullable: true},
			{Name: "mobile", Type: DB_NVarchar, Length: 255, Nullable: true},
			{Name: "password", Type: DB_NVarchar, Length: 255, Nullable: true},
			{Name: "salt", Type: DB_NVarchar, Length: 50, Nullable: true},
			{Name: "rands", Type: DB_NVarchar, Length: 50, Nullable: true},
			{Name: "company", Type: DB_NVarchar, Length: 255, Nullable: true},
	...
	mg.AddMigration("create user table v3", NewAddTableMigration(userV3))
	addTableIndicesMigrations(mg, "v3", userV3)

	//------- copy data from v1 to v2 -------------------
	mg.AddMigration("copy data_source v2 to v3", NewCopyTableDataMigration("user", "user_v2", map[string]string{
		"version":        "version",
		"login":          "login",
		"email":          "email",
		"name":           "name",
	...
	mg.AddMigration("Drop old table user_v2", NewDropTableMigration("user_v2"))
