...
func NewEngine() {
	x, err := getEngine()
  logger := log.New("main")
  logger.Info("ms sql database engine starting..")
	if err != nil {
	...
	err = SetEngine(x)
  logger.Info("ms sql set database engine")
	if err != nil {
...
func SetEngine(engine *xorm.Engine) (err error) {
  ...
  logger := log.New("main")
  logger.Info("ms sql database set engine in setEngine method")
	x = engine
...
func getEngine() (*xorm.Engine, error) {
  ...
  case "mssql":
    logger := log.New("main")
    logger.Info("ms sql database inialization")
      cnnstr=fmt.Sprintf("server=%s;user id=%s;password=%s;database=%s",
      DbCfg.Host,DbCfg.User,DbCfg.Pwd,DbCfg.Name)
  ...
  sqlog.Info("Initializing DB", "dbname", DbCfg.Name)
  sqlog.Info("Initializing DB", "dbname", DbCfg.Host)
...
func LoadConfig() {
  ...
  logger := log.New("main")
  logger.Info("ms sql database loading")
  ...
  logger.Info("ms sql database name %s",DbCfg.Name)
  ...
  if DbCfg.Type == "mssql" {
    logger := log.New("main")
    logger.Info("ms sql database key asses")
    mssqlConfig.SslMode = DbCfg.SslMode
    mssqlConfig.CaCertPath = sec.Key("ca_cert_path").String()
    mssqlConfig.ClientKeyPath = sec.Key("client_key_path").String()
    mssqlConfig.ClientCertPath = sec.Key("client_cert_path").String()
    mssqlConfig.ServerCertName = sec.Key("server_cert_name").String()
  }
}
