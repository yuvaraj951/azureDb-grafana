...
func (mg *Migrator) Start() error {
	...
	if err != nil {
    mg.Logger.Info(" DB migration error")
		return err
	}
  mg.Logger.Info(" DB migration got..")
    ...
    mg.Logger.Info("getting DB Log migration")
		record := MigrationLog{
...
func (mg *Migrator) exec(m Migration) error {
    ...
    mg.Logger.Info("Executing session", "id", m.Id())
		condition := m.GetCondition()
		if condition != nil {
			sql, args := condition.Sql(mg.dialect)
      mg.Logger.Info("Executing condition.Sql method", "id", m.Id())
...
func (mg *Migrator) inTransaction(callback dbTransactionFunc) error {
	var err error
  mg.Logger.Info("inTransaction method exec")
